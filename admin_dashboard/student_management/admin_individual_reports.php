...
<body onload="get_ay();get_stud_id('')">
  ...
  <ul class="sidebar-nav" id="sidebar-nav">
    ...
    <?php 
       $select = "SELECT *
           ...
           INNER JOIN tblstudents AS s ON s.autoid = gss.studentID
           LEFT JOIN tblgrades AS g ON g.grade_sectID = gss.gradesectID
           INNER JOIN tblclass_schedules_teachers as tst on tst.cstid=gss.class_schedule_id
          WHERE cst_teachersid='$_GET[cst_teachersid]' ORDER BY last_name ASC";
      ...
      <ul id="students-nav" class="nav-content" data-bs-parent="#sidebar-nav">
        <li onclick="get_stud_id(\''.$rowselect['autoid'].'\')"><a href="#" class=" text-danger"><i class="bi bi-person-bounding-box text-primary fs-3"></i><span>'.$rowselect['last_name'].' - '.strtoupper($rowselect['first_name']).'</a></li>
      </ul>
        ';
...
<!-- ======================= -->
  ...
  <script>
    ...
    function get_stud_id(student_id) {
        ...
        $.ajax({
            ...
            data: { 
            "loading_individual_grade_reports": '1',
